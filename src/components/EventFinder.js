...
export default function EventFinder({ city }) {
    ...
    async function getData() {
      try {
        setLoading(true);
        const response = await axios.get(`https://app.ticketmaster.com/discovery/v2/events.json?size=200&city=${eventInfo.city}&startDateTime=${eventInfo.datetime}&sort=${eventInfo.sort}&apikey=`); // API key must be added, this is empty for security purposes 
    ...
    const handleSubmit = (e) => { 
      e.preventDefault();
      changeEventInfo({city: searchEvent.city, datetime: searchEvent.datetime, sort: searchEvent.sort})
    }
